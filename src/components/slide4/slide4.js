...
class Slide4 extends React.Component {
    ...
    flip = (e) => {
        ...
        if(!card[c.id.value - 1].ck) {
            if (c.name.value === "big") {
                card[c.id.value - 1].class = "karta-front-veliko-a flip-vertical-right";
                card[c.id.value - 1].ck = true
                this.state.arr.push(card[c.id.value - 1])
            }
            if (c.name.value === "small") {
                card[c.id.value - 1].class = "flip-vertical-right karta-front-malo-a";
                card[c.id.value - 1].ck = true
                this.state.arr.push(card[c.id.value - 1])
            }
        ...
        if(this.state.arr.length === 2){
            if(this.state.arr[0].value === this.state.arr[1].value){
                ...
                if(card[this.state.arr[0].id - 1].value === "big"){
                    card[this.state.arr[0].id - 1].class = "karta-front-veliko-a pulsate-bck";
                    card[this.state.arr[1].id - 1].class = "karta-front-veliko-a pulsate-bck";
                }
                if(card[this.state.arr[0].id - 1].value === "small"){
                    card[this.state.arr[0].id - 1].class = "karta-front-malo-a pulsate-bck";
                    card[this.state.arr[1].id - 1].class = "karta-front-malo-a pulsate-bck";
                }
            } else {
                let id1 = this.state.arr[0].id;
                let id2 = this.state.arr[1].id;
                setTimeout( () => {
                    card[id1 - 1].class = "karta-back-ljubicasta"
                    card[id2 - 1].class = "karta-back-ljubicasta"
                    card[id1 - 1].ck = false
                    card[id2 - 1].ck = false
